...
void Triangle::Initialize() {
	...
	worldTransform->CreateTransformationResource(*dx_);
	CreateBufferView();
...
void Triangle::Update() {
	...
	// 三角形を動かす
	worldTransform->scale_.y += 0.03f;

	// 行列の更新
	worldTransform->UpdateMatrix(*mainCamera_);
}
...
void Triangle::Draw() {
	...
	//wvp用のCBufferの場所を指定
	dx_->commandList->SetGraphicsRootConstantBufferView(1,worldTransform->GetConstBuffer()->GetGPUVirtualAddress());
