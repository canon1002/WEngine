...
void SpriteCommon::CreateRootSignature()
{
	...
	dx_->hr = D3D12SerializeRootSignature(&descriptionRootSignature,
	...
	// バイナリを元に
	dx_->hr = dx_->device_->CreateRootSignature(0, signatureBlob->GetBufferPointer(),
		signatureBlob->GetBufferSize(), IID_PPV_ARGS(&rootSignature));
	assert(SUCCEEDED(dx_->hr));
}
...
void SpriteCommon::CreateGraphicsPipeline()
{
	...
	// 実際に生成
	dx_->hr = dx_->device_->CreateGraphicsPipelineState(&graphicsPipelineStateDesc,
...
void SpriteCommon::DrawBegin()
{
	...
	dx_->commandList->SetGraphicsRootSignature(rootSignature.Get());
	dx_->commandList->SetPipelineState(graphicsPipelineState.Get());
}
