...
void Player::Avoid()
{
	...
	if (mBehavior != Behavior::kAvoid &&
		...
		if (mInput->GetStick(Gamepad::Stick::LEFT_X) < -stickValue || // 左 
			...
			// カメラの回転量を反映
			direction = TransformNomal(direction, MainCamera::GetInstance()->GetWorld().GetWorldMatrix());
			// y座標は移動しない
			direction.y = 0.0f;
...
void Player::Move()
{
	...
	if (mBehavior == Behavior::kRoot || mBehavior == Behavior::kGuard) {
		...
		// 平行移動を行う
		mObject->AddTranslate(direction * moveSpeed);
...
void Player::DebagCtr()
{
	...
	// メインカメラに追加の平行移動値を与える
	MainCamera::GetInstance()->SetAddTranslation(TransformNomal(cVel, MainCamera::GetInstance()->GetWorld().GetWorldMatrix()));
