...
void BossEnemy::Init() {
	...
	// 拡大率を変更
	mWeapon->GetWorld().scale = { 4.0f,4.0f,20.0f };

	// ペアレント
	mWeapon->GetWorld().SetParent(mRightHandWorldMat);
...
void BossEnemy::Update() {
	...
	mRightHandWorldMat = Multiply(
		...
		].skeletonSpaceMatrix, GetObject3D()->GetWorld().GetWorldMatrix());
	...
	mWeaponWorldMat[0] = Multiply(
			...
			].skeletonSpaceMatrix, mWeapon->GetWorld().GetWorldMatrix());
...
void BossEnemy::DrawGUI() {
	...
	// 武器のワールド行列を取得
	Matrix4x4 matrix = mWeapon->GetWorld().GetWorldMatrix();
	...
	for (int i = 0; i < 4; ++i) {
		// Floatの4x4行列内の数値を表示
		ImGui::DragFloat4(("Row " + std::to_string(i)).c_str(), mWeapon->GetWorld().GetWorldMatrix().m[i]);
	}
