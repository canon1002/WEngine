...
void ModelManager::LoadModel(const std::string& directoryPath, const std::string& filepath){
	...
	//初期化
	model.reset(Model::Create(directoryPath, filepath));
...
std::shared_ptr<Model> ModelManager::Create(const std::string& filepath, const std::string filename){
	...
	if (sModels_.find(filepath) == sModels_.end()) {
		...
		sModels_[filepath].reset(Model::Create(filepath, filename));
	}
	// モデルのポインタを返す
...
void ModelManager::CreateRootSignatureForShadow()
{
	...
	// VSでShadow関連で使用する レジスタ番号(b1)
	rootParameters[6].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV; // CBVを使う 
	rootParameters[6].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX; // VertexShaderで使う
	rootParameters[6].Descriptor.ShaderRegister = 1; // レジスタ番号とバインド 
...
void ModelManager::CreateGraphicsPipelineForShadow()
{

	// ルートシグネチャを生成する
	CreateRootSignatureForShadow();
	...
	// PSOを生成する(P.38)
	D3D12_GRAPHICS_PIPELINE_STATE_DESC graphicsPipelineStateDesc{};
	graphicsPipelineStateDesc.pRootSignature = rootSignatureForForShadow.Get();
	graphicsPipelineStateDesc.InputLayout = inputLayoutDesc;
