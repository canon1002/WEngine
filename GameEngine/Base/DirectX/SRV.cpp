...
int SRV::LoadTexture(const std::string filePath) {
	...
	// テクスチャを読んで転送する
	DirectX::ScratchImage mipImages = Texture::LoadTextrue(filePath);
	textureData.metadata = mipImages.GetMetadata();
	textureData.textureResource = Texture::CreateTextureResource(mDxCommon->device_, textureData.metadata);
	textureData.intermediaResource = Texture::UpdateTextureData(mDxCommon,textureData.textureResource, mipImages);
