...
#include "Effect/Particle/ParticleManager.h"
...
void TitleScene::Init() {
	...
	mCamera->Init("MainCamera");
	mCamera->GetWorld().rotation = {0.524f ,0.0f,0.0f};
	mCamera->GetWorld().translation = {0.0f,80.0f ,-160.0f};
	...
	// カメラ回転
	mCameraRot.s = { mCamera->GetWorld().rotation.x,0.0f,0.0f };
	mCameraRot.e = { 0.05f,0.0f,0.0f };
	mCameraRot.t = 0.0f;

	// カメラ移動
	mCameraTrYZ.s = mCamera->GetWorld().translation;
	...
	// ビネット初期設定(透明)
	PostEffects* render = PostEffects::GetInstance();
...
void TitleScene::Update() {
	...
	if (!mIsActiveTransition) {
		...
		if (mInput->GetPused(Gamepad::Button::B) || mInput->GetTriggerKey(DIK_RETURN)) {
			...
			// 選択画面でゲーム開始を選択したら遷移後にシーン移行
				...
				// 遷移のスタート地点設定
				mCameraTrZ.s = mCamera->GetWorld().translation;
			}
	...
	if (mIsTransitionTitleSelect && !mIsTransitionGameScene) {
		...
		// 補間後の数値を計算
		mCamera->GetWorld().rotation = ExponentialInterpolation(mCameraRot.s, mCameraRot.e, mCameraRot.t, 1.0f);
		...
		// 補間後の数値を計算
		mCamera->GetWorld().translation = ExponentialInterpolation(mCameraTrYZ.s, mCameraTrYZ.e, mCameraTrYZ.t, 1.0f);
	...
	if (mIsTransitionGameScene) {
		...
		// 補間後の数値を計算
		mCamera->GetWorld().translation = ExponentialInterpolation(mCameraTrZ.s, mCameraTrZ.e, mCameraTrZ.t, 1.0f);
		// 画面をどんどん暗くする
		PostEffects::GetInstance()->SetViggnetIndex(ExponentialInterpolation(0.0f, 10.0f, mCameraTrZ.t, 1.0f));
...
void TitleScene::Draw() {
	...
	//mObject->Draw();
	mGroundObj->Draw(*mCamera);
	mBuildingObj->Draw(*mCamera);
}
