...
void Skinning::ApplyAniation() {
		...
		for (Joint& joint : mSkeleton.joints)
		{
			...
			if (auto it = mPreSkincluster->animation.nodeAnimations.find(joint.name); it != mPreSkincluster->animation.nodeAnimations.end())
			{
				if (auto itNext = mCurrentSkinCluster->animation.nodeAnimations.find(joint.name); itNext != mCurrentSkinCluster->animation.nodeAnimations.end())
				{
					...
					joint.transform.translation = Lerp(transration[0], transration[1], mMotionBrendingTime);
					joint.transform.rotation = Slerp(rotation[0], rotation[1], mMotionBrendingTime);
					joint.transform.scale = Lerp(scale[0], scale[1], mMotionBrendingTime);
				}
			}
	...
	// 上記以外
		...
		for (Joint& joint : mSkeleton.joints)
		{
			...
			{
				const NodeAnimation& rootAnimation = (*it).second;
				joint.transform.translation = CalculateValue(rootAnimation.translation, mCurrentSkinCluster->animationTime);
				joint.transform.rotation = CalculateValue(rootAnimation.rotation, mCurrentSkinCluster->animationTime);
				joint.transform.scale = CalculateValue(rootAnimation.scale, mCurrentSkinCluster->animationTime);
			}
		}
	}
}
