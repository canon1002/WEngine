...
void GaussianFilter::CreateEffectResource() {
	...
	// リソース生成
	mEffectResource = DirectXCommon::GetInstance()->CreateBufferResource(DirectXCommon::GetInstance()->mDevice.Get(), sizeof(GaussianFilterItems));
	// 書き込むためのアドレスを取得
...
void GaussianFilter::CreateGraphicsPipeline(){
	...
	Microsoft::WRL::ComPtr<IDxcBlob> pixelShaderBlob = WinApp::CompileShader(L"Shaders/PostEffect/GaussianFilter.PS.hlsl",
